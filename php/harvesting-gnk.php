...
$cats['wms']['category']=1;
$cats['wms']['datasetCategory']=2;

$cats['wfs']['category']=3;
$cats['wfs']['datasetCategory']=4;

$cats['wcs']['category']=5;
$cats['wcs']['datasetCategory']=6;

$cats['wps']['category']=7;
$cats['wps']['datasetCategory']=10;

$cats['sos']['category']=8;
$cats['sos']['datasetCategory']=9;

//$cats['wmts']['category']=10;
//$cats['wmts']['datasetCategory']=11;

//$cats['csw']['category']=12;
//$cats['csw']['datasetCategory']=0;

require 'settings.php';
// CURL COOKIE PATH
$cpath = "/var/www/ogcwxs/cookie.txt";
// AUTHENTIFIKACIA
$auth_url="http://192.168.1.246:8080/geonetwork/j_spring_security_check"; 
...
<html xmlns="http://www.w3.org/1999/xhtml"> 
	<head>
		...
		<script>
			...
			function vec()
			{
				document.location.href = 'harvesting.php';
			}
		</script>
	...
	<body onload="refresh()">
		...
		<div id="content">
			<?php 
				$now=time();
				// BEZI / NEBEZI
				$q=dbq("SELECT  * FROM services WHERE status=3");
				if(mysql_num_rows($q))
				{
					
					// bezi
					$r=mysql_fetch_assoc($q);
					echo "Running harvesting task for the OGC endpoint ID: {$r['id']}<br><br>";
					$url="http://192.168.1.246:8080/geonetwork/srv/eng/xml.harvesting.get";
					$xml = 
"<request><id>{$r['id_gn']}</id></request>";
					$ch = curl_init($url);
					curl_setopt($ch, CURLOPT_COOKIEJAR, $cpath); 
					curl_setopt($ch, CURLOPT_COOKIEFILE, $cpath); 
					...
					{
						// TK status zmneny na 5, aby ulohy prebehli iba raz
						dbq("UPDATE services SET harvested=1, time_harvest=$now WHERE id = {$r['id']}");
				...
				else
				{
					...
					// bol / nebol pridany
					$q=dbq("SELECT * FROM services WHERE status=1 AND id_gn=0 AND harvested=0 AND type!='csw' ORDER BY id ASC LIMIT 1");
					if(mysql_num_rows($q))
					{
						...
						$url = "http://192.168.1.246:8080/geonetwork/srv/eng/xml.harvesting.add";
						...
						else
						{
							...
							$url = "http://192.168.1.246:8080/geonetwork/srv/eng/xml.harvesting.run";
					...
					else
					{
						...
						$r=mysql_fetch_assoc(dbq("SELECT * FROM services WHERE status=1 AND id_gn>0 ORDER BY time_harvest ASC LIMIT 1"));
						$nodeId= $r['id_gn'];
