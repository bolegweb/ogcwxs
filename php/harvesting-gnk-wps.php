...
$cats['wps']['category']=1;
$cats['wps']['datasetCategory']=2;

require 'settings.php';
// CURL COOKIE PATH
$cpath = "/usr/share/ogcwxs/cookie/wps.txt";
// AUTHENTIFIKACIA
$auth_url="http://31.147.204.167:8080/gnk_wps/j_spring_security_check"; 
...
<html xmlns="http://www.w3.org/1999/xhtml"> 
	<head>
		...
		<script>
			...
			function vec()
			{
				document.location.href = 'harvesting-wps.php';
			}
		</script>
	...
	<body onload="refresh()">
		...
		<div id="content">
			<?php 
				$now=time();
				// BEZI / NEBEZI
				$q=dbq("SELECT * FROM services WHERE type='wps' AND status=3");
				if(mysql_num_rows($q))
				{
					...
					$url="http://31.147.204.167:8080/gnk_wps/srv/eng/xml.harvesting.get";
				...
				else
				{
					...
					// bol / nebol pridany
					$q=dbq("SELECT * FROM services WHERE status=1 AND type='wps' AND harvested=0 ORDER BY id ASC LIMIT 1");
					if(mysql_num_rows($q))
					{
						...
						$url = "http://31.147.204.167:8080/gnk_wps/srv/eng/xml.harvesting.add";
						...
						else
						{
							...
							$url = "http://31.147.204.167:8080/gnk_wps/srv/eng/xml.harvesting.run";
